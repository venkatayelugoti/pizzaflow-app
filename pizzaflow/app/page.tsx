...
export default function NextAppPage() {
  ...
  // Load recent orders from Next.js API route
  const fetchRecentOrders = async () => {
    setIsLoadingOrders(true);
    try {
      const res = await fetch('/api/orders'); // Or fetch direct if needed, or through /api/submit-order dependencies
      if (res.ok) {
  ...
  const calculateBill = (basePrice: number, pizzaPrice: number, toppingPrice: number, quantity: number) => {
  ...
  const validateForm = (showError = false) => {
    if (!customer || typeof customer !== 'string') {
      if (showError) setValidationError('Customer name is required.');
      return false;
    }
    const cleanName = customer.trim();
    if (cleanName.length < 2 || cleanName.length > 40) {
      if (showError) setValidationError('Customer name must be between 2 and 40 characters.');
      return false;
    }
    if (!/^[a-zA-Z\s]+$/.test(cleanName)) {
      if (showError) setValidationError('Customer name can only contain letters and spaces.');
      return false;
    }

    if (!phone) {
      if (showError) setValidationError('Phone number is required.');
      return false;
    }
    if (!/^[6-9]\d{9}$/.test(phone)) {
      if (showError) setValidationError('Phone number must be exactly 10 digits and start with 6, 7, 8, or 9.');
      return false;
    }

    const qtyVal = Number(quantity);
    if (!Number.isInteger(qtyVal) || qtyVal < 1 || qtyVal > 10) {
      if (showError) setValidationError('Quantity must be an integer between 1 and 10.');
      return false;
    }
